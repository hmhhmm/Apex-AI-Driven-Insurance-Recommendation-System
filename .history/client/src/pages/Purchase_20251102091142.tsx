...
const Purchase = () => {
  ...
  const calculateAge = (dateOfBirth: string): number => {
    if (!dateOfBirth) return 29 // default
  ...
  // Helper function to determine lifestyle from exercise frequency
  const determineLifestyle = (exerciseFrequency: string): string => {
    if (!exerciseFrequency) return "Moderate"
  ...
  const getAssessmentData = () => {
    try {
      const saved = localStorage.getItem('apex-user-assessment')
      if (saved) {
  ...
  const userSummary = {
    age: assessmentData?.dateOfBirth ? calculateAge(assessmentData.dateOfBirth) : 29,
    gender: assessmentData?.selectedGender === 'female' ? "Female" : assessmentData?.selectedGender === 'male' ? "Male" : selectedGender === 'female' ? "Female" : selectedGender === 'male' ? "Male" : "Female",
    lifestyle: assessmentData?.exerciseFrequency ? determineLifestyle(assessmentData.exerciseFrequency) : "Active",
